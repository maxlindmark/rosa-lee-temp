...
```{r}
plot_rosa_lee_faceted <- function(pop_all_species) {
  species_list <- c("cod", "flatfish", "sardine")

  # Process species
  all_data <- map(species_list, function(species_name) {
    lh <- get_lh(species_name)

    comparison <- pop_all_species |>
      filter(
        species == species_name,
        year >= n_years - 10,
        scenario %in% c("Fished_NoWarming", "Unfished_NoWarming")
      ) |>
      select(scenario, age, platoon, N, K, Fishing, Warming)

    mean_K <- lh |> pull(mean_K)

    # Calculate weighted mean and SD for K by age
    k_summary <- comparison |>
      filter(
        scenario == "Fished_NoWarming",
        age <= 10
      ) |>
      summarise(
        K_weighted = sum(K * N) / sum(N),
        # Weighted SD: sqrt(sum(w * (x - weighted_mean)^2) / sum(w))
    ...
    get_mean_growth <- function(fishing) {
      comparison |>
        filter(Fishing == fishing, between(age, 4, 10)) |>
        summarise(K_weighted = sum(K * N) / sum(N)) |>
        crossing(age = 0:10) |>
        mutate(
          Fishing = fishing,
          length = lh$Linf * (1 - exp(-K_weighted * (age - lh$t0)))
        )
    }

    mean_growth <- bind_rows(
      get_mean_growth("Fished"),
      get_mean_growth("Unfished")
    ) |>
      mutate(species = species_name)

    list(
      k_distributions = k_summary,
      mean_growth = mean_growth
    )
  })

  k_dist_all <- map_dfr(all_data, "k_distributions")
  mean_growth_all <- map_dfr(all_data, "mean_growth")

  # Clean species names
  k_dist_all$facet_species <- str_to_sentence(k_dist_all$species)
  mean_growth_all$facet_species <- str_to_sentence(mean_growth_all$species)
...
# Note the reason cod has higher K for earlier ages I think has to do with cod not really being at equilibrium, in contrast to the other species!
```
