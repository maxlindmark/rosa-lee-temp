...
# This is just the weighed mean, sample proporitonal to N
...
# Here we sample a fixed number by age class. This more reflects how sampling is not, and gives less influence to changes in demography (age structure, but still accounts for platoon structure)
slopes <- pop_all_species_seeds |>
  group_by(species, year, scenario, seed, age) |>
  group_modify(~ {
    # sampling probabilities within age
    probs <- .x$N / sum(.x$N)

    # draw equal number of individuals at this age
    K_samp <- sample(
      .x$K,
      size = 1000,
      replace = TRUE,
...
# t <- pop_all_species_seeds |> 
#   filter(species == "cod" & scenario == "Fished_NoWarming", age <= 10)
# t2 <- t |> 
#   summarise(
#     k_wt = sum(K * N) / sum(N),
#     km = mean(K),
#     .by = c(age, year)
#   )
# ggplot(t2, aes(year, k_wt, color = as.factor(age))) + geom_line() + 
#   facet_wrap(~age, scales = "free") +
#   guides(color = "none")
# t2 |> filter(year == 1)

mean_k_yr1 <- pop_all_species_seeds |> 
  filter(year == 1 & scenario %in% c("Fished_NoWarming")) |> 
  # Compare to first year of fishing instead of the mean in the parameter distribution
  # If comparing against mean in distribution it can look like it's increasesing in the unfished scenario
  summarise(mean_K = mean(K), .by = species) |> 
  mutate(facet_species = str_to_sentence(species),
         label = letters[1:3])
...
mean_growth_all <- comparison_data |>
  ...
  mutate(
    ...
    facet_species = str_to_sentence(species),
    Fishing = ifelse(Fishing == "Yes", "Fished", "Unfished")
  )
...
warming_values <- seq(0, 2, length.out = 10)
F_values <- seq(0, 0.4, length.out = 10)
n_seeds <- 30
...
results_all_params <- map_dfr(1:nrow(param_grid), function(i) {
  ...
  # Calculate weighted K within each age, then equal weight across ages
  pop_all_species |>
...
# Define parameter ranges to run
# 1000 iterations takes 5.65 h
warming_values <- 0#seq(0, length.out = 15)
F_values <- seq(0, 0.4, length.out = 5)
n_seeds <- 20
...
tic()
results_all_params_test <- map_dfr(1:nrow(param_grid), function(i) {
  warming <- param_grid$total_warming[i]
  F_val <- param_grid$F_longterm[i]
  seed_val <- param_grid$seed[i]
...
###
slopes <- results_all_params_test |>
  mutate(scenario = paste(species, F_longterm, total_warming, scenario, seed, sep = "-")) %>%
  split(.$scenario) |>
  map_dfr(~ {
    fit <- lm(K ~ temp, data = .)
...
slopes |>
  ...
  summarise(mean = mean(estimate), .by = c(species, F_longterm, seed)) |>
  ggplot(aes(as.numeric(F_longterm), mean)) +
  facet_wrap(~species) +
...
slopes |>
  filter(total_warming == min(total_warming) & scenario == "Fished_Warming_full") |>
  summarise(mean = mean(estimate), .by = c(species, F_longterm)) |>
  ggplot(aes(as.numeric(F_longterm), mean)) +
  facet_wrap(~species) +
  geom_jitter(height = 0, width = 0.05) +
  geom_smooth(method = "lm") +
...
# p2
slopes |>
  filter(total_warming == min(total_warming)) |> 
  summarise(
    estimate = mean(estimate),
  ...
  filter(scenario %in% c("Fished_Warming_noRL", "Fished_Warming_full")) |>
  mutate(facet = ifelse(scenario == "Fished_Warming_noRL", "No Rosa Lee", "With Rosa Lee")) |>
  ggplot(aes(F_longterm, estimate, color = str_to_sentence(species)),
         linetype = str_to_sentence(species)) +
  #geom_line() +
  geom_point() +
